...
export function TemplateModal({ onInsert, onClose, titulo }) {
  ...
  useEffect(() => {
    function onKey(e) {
      // Escape handled by Modal base; we only handle navigation + enter
  ...
  return (
    <Modal open={true} onClose={onClose} size="sm">
      <div className="flex items-center justify-between px-4 py-3 border-b border-bdr dark:border-bdr-dark flex-shrink-0">
        <span className="text-sm font-medium text-ink dark:text-ink-dark">
          Inserir template
        </span>
        <div className="flex items-center gap-2">
          <kbd className="text-[10px] text-ink-3 dark:text-ink-dark3 bg-bg dark:bg-bg-dark px-1.5 py-0.5 rounded-soft border border-bdr dark:border-bdr-dark">↑↓</kbd>
          <kbd className="text-[10px] text-ink-3 dark:text-ink-dark3 bg-bg dark:bg-bg-dark px-1.5 py-0.5 rounded-soft border border-bdr dark:border-bdr-dark">↵ inserir</kbd>
          <button onClick={onClose} className="text-ink-3 dark:text-ink-dark3 hover:text-ink dark:hover:text-ink-dark text-xs ml-1 transition-colors">✕</button>
        </div>
      </div>
      ...
      {!loading && templates.length > 0 && (
        <div className="px-4 py-2 border-t border-bdr-2 dark:border-bdr-dark2 flex-shrink-0">
